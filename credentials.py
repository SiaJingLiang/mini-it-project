...
def minus_amount():
    ...
    find = c.execute('SELECT BOOK_AMOUNT from TEST WHERE BOOK_ID=?', (book_id,))
    for x in find: 
        amount = x[0]
    minus_amount = int(input('Enter amount minus: '))  
    while amount - minus_amount < 0:
        print('Invalid amount to minus.')
        minus_amount = int(input('Enter amount minus: '))
    final_amount = x[0] - minus_amount
    c.execute("""UPDATE TEST SET BOOK_AMOUNT = ? WHERE BOOK_ID = ?""", (final_amount, book_id,))
